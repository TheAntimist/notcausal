	...
	if deps, ok := d.deferred[storeObject.key]; ok {
		...
		for i, obj := range deps {
			...
			if len(obj.dep) == 1 && dep.Key == storeObject.key &&
				...
				(dep.Version.Equals(curVersion) || dep.Version.LessThan(curVersion)) {
				// Empty deps for this
				obj.dep = nil
				d.recurReplicatedWrites(*obj)
				u = append(u[:i], u[i+1:]...)
			}
		}
		if len(u) == 0 {
...
func (d* DataCenter) replicatedWrite(p Packet, remoteAddr string) (Packet, bool) {
	...
	if p.Message["has_dep"].(bool) {
		for _, d := range p.Message["dep"].([]interface{}) {
			dependency := d.(map[string]interface{})
			depVersion := dependency["version"].(map[string]interface{})
			storeObject.dep = append(storeObject.dep, Dependency{
	...
	if len(storeObject.dep) > 0 && !d.validDependencies(storeObject.dep) {
		fmt.Printf(" [debug] Deferring request of %s <%d, %d> as dependency %s is not valid\n",
			storeObject.key, storeObject.version.Timestamp, storeObject.version.DCId,
			storeObject.dep)
		d.storeMut.Lock()
		for _, dep := range storeObject.dep {
			if curDeps, ok := d.deferred[dep.Key]; ok {
				d.deferred[dep.Key] = append(curDeps, &storeObject)
			} else {
				newDeps := make([]*StoreObject, 0)
				d.deferred[dep.Key] = append(newDeps, &storeObject)
			}
		}
		d.storeMut.Unlock()
		return Packet{
...
// depCheck
func (d *DataCenter) validDependencies(deps []Dependency) bool {
	d.storeMut.RLock()
...
func (d *DataCenter) showDep(args []string) {
	...
	d.storeMut.RLock()
	for _, c := range d.curDep {
		table.Append([]string{c.Key, fmt.Sprintf("<%d, %d>", c.Version.Timestamp, c.Version.DCId)})
	}
	d.storeMut.RUnlock()
	table.Render()
...
func (d *DataCenter) updateDependency(dep Dependency) {
	found := false
	foundIndex := -1
	for i, c := range d.curDep {
		if c.Key == dep.Key {
			found = true
			foundIndex = i
		}
	...
	if found {
		d.curDep[foundIndex] = dep
	} else {
		d.curDep = append(d.curDep, dep)
	}
}
